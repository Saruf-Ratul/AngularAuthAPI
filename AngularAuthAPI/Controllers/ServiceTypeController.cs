...
using ServiceType = AngularAuthAPI.Models.ServiceType;
...
namespace AngularAuthAPI.Controllers
{
    ...
    public class ServiceTypeController : ControllerBase
    {
        ...
        public ServiceTypeController(IServiceType ServiceTypeService, AppDbContext db)
        {
        ...
        public IActionResult GetAll()
        {
            try
            {
                var data = _ServiceTypeService.GetAll(_db);
        ...
        [HttpPost("add")]
        public IActionResult addData(ServiceType model)
        {
            using (var dbTransaction = _db.Database.BeginTransaction())
            {
                try
                {
                    var data = _ServiceTypeService.addData(model, _db);
                    if (data)
                    {
        ...
        {
            var data = _ServiceTypeService.Delete(id);
            if (data)
            {
                returnObj.IsExecuted = true;
